...
public class PipeModelsProvider implements DataProvider {
    ...
    private void registerPipeModels(DataWriter cache) throws IOException {
    ...
    private void registerPipeModel(DataWriter cache, PipeBlock pipe, String texture, String connectionTexture, boolean transparent)
            throws IOException {
        ...
        DataProvider.writeToPath(cache, modelJson, baseFolder.resolve("main.json"));
    }

    /**
     * Register a simple textures pipe part model, and return the id of the model.
     */
    private String registerPipePart(DataWriter cache, Path baseFolder, PipeBlock pipe, String kind, String texture, boolean transparentSuffix)
            throws IOException {
        ...
        DataProvider.writeToPath(cache, obj, baseFolder.resolve(kind + ".json"));

        var id = "pipe/%s/%s".formatted(pipe.id, kind);
        return MdId.of(id).toString();
    }

    private void registerAttachments(DataWriter cache) throws IOException {
        // Register each model.
     ...
     */
    private void registerAttachment(DataWriter cache, RenderedAttachment attachment, String texture) throws IOException {
        var obj = new JsonObject();
        ...
        DataProvider.writeToPath(cache, obj,

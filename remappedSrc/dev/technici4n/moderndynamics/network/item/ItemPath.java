...
public class ItemPath {
    ...
    public NetworkNode<ItemHost, ItemCache> getStartingPoint(ServerWorld level) {
        return ItemHost.MANAGER.findNode(level, startingPos.offset(path[0]));
    }
    ...
    @Nullable
    AttachedAttachment getEndAttachment(ServerWorld level) {
        var lastNode = ItemHost.MANAGER.findNode(level, targetPos.offset(getTargetBlockSide()));
     ...
     */
    Predicate<ItemVariant> getEndFilter(ServerWorld level) {
        var endPipe = targetPos.offset(getTargetBlockSide());

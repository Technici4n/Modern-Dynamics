...
import dev.technici4n.moderndynamics.network.NetworkNode.Connection;
...
public class ItemHost extends NodeHost {
    ...
    public void tickAttachments() {
        ...
        for (var side : Direction.values()) {
            ...
            if (attachment instanceof ItemAttachedIo itemAttachedIo && itemAttachedIo.isEnabledViaRedstone(pipe)) {
                if (currentTick - lastOperationTick[side.getId()] < itemAttachedIo.getItemOperationTickDelay())
                    continue;
                ...
                if (itemAttachedIo.getType() == IoAttachmentType.EXTRACTOR) {
                    ...
                    if (itemAttachedIo.isStuffed()) {
                        ...
                        var insertTarget = SimulatedInsertionTargets.getTarget(pipe.getWorld(), pipe.getPos().offset(side),
                                side.getOpposite());
                        ...
                        for (var path : paths) {
                            var extractTarget = ItemStorage.SIDED.find(pipe.getWorld(), path.targetPos, path.getTargetBlockSide());
    ...
    public void clientTick() {
        for (var it = clientTravelingItems.iterator(); it.hasNext();) {
            ...
            if (MathHelper.fractionalPart(travelingItem.traveledDistance) < travelingItem.speed()) {
                // Goes out of this pipe!
                it.remove();
                if (travelingItem.traveledDistance < travelingItem.totalPathDistance) {
                    // Add to next pipe
                    if (getLevel().getBlockEntity(getPos().offset(travelingItem.out)) instanceof PipeBlockEntity otherPipe) {

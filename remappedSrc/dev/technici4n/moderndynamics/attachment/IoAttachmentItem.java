...
public class IoAttachmentItem extends AttachmentItem {
    ...
    @Override
    public AttachedAttachment createAttached(NodeHost host, NbtCompound configTag) {
        if (host instanceof ItemHost) {
            return new ItemAttachedIo(this, configTag, host.getPipe()::markDirty);
        } else {
            return new FluidAttachedIo(this, configTag, host.getPipe()::markDirty);
        }
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World level, List<Text> tooltipComponents, TooltipContext isAdvanced) {
        tooltipComponents
                .add(Text.translatable("gui.moderndynamics.tooltip.attachment_upgrades").setStyle(Style.EMPTY.withColor(Formatting.GOLD)));
        /*

...
public final class ItemCachedFilter {
    ...
    private static boolean matchDamageIgnoreRest(@Nullable NbtCompound a, @Nullable NbtCompound b) {
        return getDamage(a) == getDamage(b);
    }

    private static boolean matchIgnoringDamage(@Nullable NbtCompound a, @Nullable NbtCompound b) {
        if (a == b) {
            return true;
        }
    ...
    private static int getDamage(NbtCompound tag) {
        return tag == null ? 0 : tag.getInt(ItemStack.DAMAGE_KEY);
    }

    @FunctionalInterface
    interface NbtMatcher {
        boolean matches(@Nullable NbtCompound a, @Nullable NbtCompound b);
    }
}

...
public abstract class AttachedIo extends AttachedAttachment {
    ...
    public AttachedIo(AttachmentItem item, NbtCompound configData, Runnable setChangedCallback) {
    ...
    public ItemStack removeUpgrade(int slot, int count) {
        return Inventories.splitStack(upgradeContainer.upgrades, slot, count);
    }
    ...
    protected static <T extends Enum<T>> T readEnum(T[] enumValues, NbtCompound tag, String key, T defaultValue) {
    ...
    protected static <T extends Enum<T>> void writeEnum(T enumValue, NbtCompound tag, String key) {
    ...
    public boolean isEnabledViaRedstone(PipeBlockEntity pipe) {
        ...
        var signal = pipe.getWorld().isReceivingRedstonePower(pipe.getPos());
        if (signal) {
    ...
    public void onPlaced(PlayerEntity player) {
        if (WrenchHelper.isWrench(player.getOffHandStack())) {
